...
const config = path.resolve(
  value('--remote-config', path.join(os.homedir(), '.config', 'board-watcher', 'remote.yaml')),
);
const checks = [
  ['git', 'Git', ['--version']],
  ['ssh', 'OpenSSH', ['-V']],
...
console.log(`${repoReady ? 'OK' : 'MISSING'} board-watcher checkout — ${repo}`);
console.log(`${fs.existsSync(config) ? 'OK' : 'PENDING'} remote config — ${config}`);
failed ||= !repoReady;

if (failed) {
  console.error('client preflight incomplete; install missing tools or clone Board Watcher, then rerun');
  process.exitCode = 1;
}
