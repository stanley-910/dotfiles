...
- `2026-07-04` — User-directed request (skips threshold): DSA review should also produce a **per-pattern HTML visualization** (clean/minimal/intuition-dense, same format each time — one template per pattern family: arrays, two pointers, trees, …) embedded in the problem note via `![[...]]` (his html-embed plugin). Built v1 two-pointer prototype (LC 11) at `04-code/training-arc/DSA/viz/`; format = claim → interactive stepper of *his* algorithm → pair-space elimination panel → ≤3 why-bullets. **Awaiting his verdict on the prototype before templating + editing SKILL.md's finished-problem routine.** Technical constraints captured in `project-dsa-viz-embeds` memory (vault-local). — (status: noted — pending user yes)
