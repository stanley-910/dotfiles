...
Before any canvas action, read `$HOME/.agents/skills/tldraw-offline/SKILL.md` completely and follow it. Your launch context may already contain the live server URL, bearer token, and open-document snapshot; otherwise use the skill's portable helpers.
